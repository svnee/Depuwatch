class Api::PoliticiansController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @politicians.to_xml(:only => [:id, :name, :start, :end, :pc_profile], :methods => [:partei, :bezierk, :active], :include => [:memberships, :votes], :skip_instruct => true, :root => 'politicians') }
  ...
  def show
    ...
    respond_to do |format|
      format.html { render :text => "Please use the API only with application/xml", :status => :see_other }
      format.xml  { render :xml => @politician.to_xml(:only => [:id, :name, :start, :end, :pc_profile], :methods => [:partei, :bezierk, :active], :include => [:memberships, :votes], :skip_instruct => true, :root => 'politician') }
