class Api::PoliticiansController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      format.html { render :text => "Please use the API only with application/xml or application/json", :status => :see_other }
  ...
  def show
    ...
    respond_to do |format|
      format.html { render :text => "Please use the API only with application/xml or application/json", :status => :see_other }
      format.xml  { render :xml => @politician.to_xml(:only => [:id, :name, :start, :end, :pc_profile], :methods => [:partei, :bezierk, :active], :include => [:memberships], :skip_instruct => true, :root => 'politician') }
